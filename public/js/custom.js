...
function submitFlightEnquiry(form) {
  ...
  var from = $(form).find('.fromdest').map(function() { return $(this).val(); }).get();
  var to = $(form).find('.todest').map(function() { return $(this).val(); }).get();
  var departureDate = $(form).find('.departdt').map(function() { return $(this).val(); }).get();
