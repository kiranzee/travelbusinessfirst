...
@section('content')
    ...
    <div class="container mt-5">
        ...
        <!-- edit Flight Form -->
        <form id="holidaydeals" action="{{ route('bestofactivity.update', $bestofactivity->id) }}" method="POST"
            ...
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-md-6"><img src="{{ asset('uploads/' . $bestofactivity->image) }}"
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="price" name="price"
                    value="{{ old('price', $bestofactivity->price) }}" required>
            </div>
            <!-- Rating Field -->
            <div class="form-group">
                <label for="rating">Rating</label>
                <input type="text" class="form-control" id="rating" name="rating"
                    value="{{ old('rating', $bestofactivity->rating) }}" required>
            </div>
            <!-- Rating Count Field -->
            <div class="form-group">
                <label for="rating_count">Rating Count</label>
                <input type="text" class="form-control" id="rating_count" name="rating_count"
                    value="{{ old('rating_count', $bestofactivity->rating_count) }}" required>
            </div>

            <!-- package_heading Field -->
            <div class="form-group">
                <label for="package_heading">package_heading</label>
                <input type="text" class="form-control" id="package_heading" name="package_heading"
                    value="{{ old('package_heading', $bestofactivity->package_heading) }}" required>
            </div>
            <!-- cancellation Field -->
            <div class="form-group">
                ...
                <select class="form-control" id="cancellation" name="cancellation" required>
                    <option value="0"
                        {{ old('cancellation', $bestofactivity->cancellation) == '0' ? 'selected' : '' }}>Yes</option>
                    <option value="1"
                        {{ old('cancellation', $bestofactivity->cancellation) == '1' ? 'selected' : '' }}>No</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="status" name="status" required>
                    <option value="active" {{ old('status', $bestofactivity->status) == 'active' ? 'selected' : '' }}>
                        Active</option>
                    <option value="inactive" {{ old('status', $bestofactivity->status) == 'inactive' ? 'selected' : '' }}>
                        Inactive</option>
                </select>
            </div>
